...
export async function save(f: string, buffer: Buffer) {
	return fs.writeFile(f, buffer)
}
...
// TOUCH
// following function will create a file if it does not exists
export async function touch(f: string) {
	return fs.writeFile(f, '')
}

// CONVERT
// following function will convert the given buffer to base64 string
export function toBase64(f: Buffer) {
